...
>
> **Актуализиран спрямо решение O-3 (основен домейн, публично пускане 20–27 октомври).**
> Това промени тежестта на две неща: списъкът с URL-и и пълните данни на каталога вече са
> **блокиращи**, а не желателни — новият сайт заменя стария, а не стои до него.
> Кратката версия за качване е в [12-legacy-upload-instructions.md](12-legacy-upload-instructions.md).
...
**Файл 4 — анонимизирана извадка от поръчки** — *по желание, не блокира.*

Историческите поръчки не се мигрират; остават четими на стария сайт. Извадката помага само да
разбера как е моделирана поръчката, ако по-късно решим друго. Пропусни я без притеснение.
...
### Два въпроса, на които дъмпът може и да не отговори

Напиши отговорите в `README.txt`. Без тях мигрираните цени ще са грешни, а каталогът — раздут.

1. **В каква валута са записани цените и с ДДС ли са?** Магазинът работи само в EUR (решение O-8).
   Ако старите цени са в лева, трябва да знам дали да ги превалутирам, или има вече заредени цени в
   евро. Грешка тук минава незабелязано през целия каталог.
2. **По какво се познава, че продукт вече не се продава?** Колона `active`, `status`, нулева
   наличност, или просто скрита категория? Пълната миграция иначе качва и спрените артикули.

## Стъпка 4 · Списък с URL-и

**Блокиращо заради решение O-3.** Новият сайт заменя стария на същия домейн, тоест всеки
индексиран адрес трябва да води до 200 или смислен 301. Списък, който изпуска страници, значи
изгубен органичен трафик — плащан с години.

Дай **колкото можеш повече** от трите източника; един е достатъчен, за да започна анализа, но за
самата 301 карта е нужно покритие, не проба.
...
**В. Пълно обхождане** — Screaming Frog SEO Spider. **Безплатната версия спира на 500 URL-а.**
Ако сайтът е по-голям, обхождането ще е непълно — тогава Search Console експортът от точка Б е
задължителен, а не пожелателен. Ако не си сигурен колко адреса има, `table_counts.txt` ще подскаже.
