...
if ($dumpFound) {
    ...
    # A dump may put an INSERT statement's values on continuation lines. The
    # first version matched only the header line, so it counted statements
    # instead of rows and wrote INSERT headers with no values at all. Codex
    # caught it: products reported 64 rows against 376 real product IDs.
    $inInsert       = $false
    $insertTable    = $null
    $stmtTuples     = 0
    $stmtWanted     = $false
    $stmtBuffer     = $null
    $stmtPendingSep = $false
    $oversized      = @{}
    $maxStatementBytes = 32MB
    ...
    try {
        ...
        while ($null -ne ($line = $reader.ReadLine())) {
            ...
            # --- INSERT statements, which may span many lines ------------------
            if (-not $inInsert -and $line -match '^\s*INSERT\s+INTO\s+[`"\[]?([A-Za-z0-9_$]+)') {
                $insertTable    = $Matches[1]
                $inInsert       = $true
                $stmtTuples     = 0
                $stmtPendingSep = $false

                if (Test-Deny $insertTable) {
                    $deniedSeen[$insertTable] = $true
                    $stmtWanted = $false
                } else {
                    $stmtWanted = (Test-Allow $insertTable) -and (-not $piiBlocked.ContainsKey($insertTable))
                }
                # Buffered, not streamed: a PII match on the last line must be
                # able to discard the whole statement. Written straight out,
                # earlier lines would already be in the file.
                $stmtBuffer = if ($stmtWanted) { New-Object System.Text.StringBuilder } else { $null }
            }

            if ($inInsert) {
                # Rows may be packed onto one line as '),(' or split so that a
                # line ends with '),' and the next opens with '('. Counting only
                # the first shape reports one row per line-broken statement.
                if ($stmtPendingSep -and $line -match '^\s*\(') { $stmtTuples += 1 }
                $stmtTuples += ([regex]::Matches($line, '\)\s*,\s*\(')).Count
                $stmtPendingSep = ($line -match '\)\s*,\s*$')

                if ($null -ne $stmtBuffer) {
                    [void]$stmtBuffer.AppendLine($line)
                    if ($stmtBuffer.Length -gt $maxStatementBytes) {
                        $stmtBuffer = $null
                        $stmtWanted = $false
                        $oversized[$insertTable] = $true
                    }
                }

                # mysqldump terminates a statement with ';' on its own last line.
                # A value ending in ';' inside quotes would not end the line.
                if ($line -match ';\s*$') {
                    $rows = $stmtTuples + 1
                    if ($rowCount.ContainsKey($insertTable)) { $rowCount[$insertTable] += $rows }
                    else { $rowCount[$insertTable] = $rows }

                    if ($stmtWanted -and $null -ne $stmtBuffer) {
                        $text = $stmtBuffer.ToString()
                        if (Test-Pii $text) {
                            $piiBlocked[$insertTable] = $true
                        } else {
                            $catalogOut.Write($text)
                            $catalogSeen[$insertTable] = $true
                        }
                    }

                    $inInsert = $false; $stmtBuffer = $null; $insertTable = $null
                }
                continue
            }
        }
    ...
    # A table can pass the PII scan on its first statements and fail on a later
    # one. By then the clean statements are already on disk, so the extract would
    # keep a partial copy of a table the scan rejected. Rewrite the file without
    # them instead of trusting the final sweep to catch it.
    $blockedButWritten = @($piiBlocked.Keys | Where-Object { $catalogSeen.ContainsKey($_) })
    if ($blockedButWritten.Count -gt 0) {
        $tmpPath = "$catalogPath.tmp"
        $purgeIn  = [System.IO.StreamReader]::new($catalogPath, [System.Text.Encoding]::UTF8, $true)
        $purgeOut = [System.IO.StreamWriter]::new($tmpPath, $false, $utf8NoBom)
        $dropping = $false
        try {
            while ($null -ne ($line = $purgeIn.ReadLine())) {
                if (-not $dropping -and $line -match '^\s*INSERT\s+INTO\s+[`"\[]?([A-Za-z0-9_$]+)') {
                    $dropping = ($blockedButWritten -contains $Matches[1])
                }
                if (-not $dropping) { $purgeOut.WriteLine($line) }
                if ($line -match ';\s*$') { $dropping = $false }
            }
        } finally {
            $purgeIn.Dispose(); $purgeOut.Dispose()
        }
        Move-Item -LiteralPath $tmpPath -Destination $catalogPath -Force
        foreach ($t in $blockedButWritten) { [void]$catalogSeen.Remove($t) }
        Warn "$($blockedButWritten.Count) table(s) failed the PII scan after partial export and were purged from catalog_data.sql"
        Note "Purged after partial export: $(($blockedButWritten | Sort-Object) -join ', ')"
    }
    ...
    if ($oversized.Count -gt 0) {
        Warn "$($oversized.Count) table(s) had an INSERT larger than $([math]::Round($maxStatementBytes/1MB)) MB and were skipped"
        Note "Skipped as oversized statements: $(($oversized.Keys | Sort-Object) -join ', ')"
    }
